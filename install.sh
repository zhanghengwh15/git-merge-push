...
check_path() {
    ...
    if [[ ":$PATH:" != *":$install_dir:"* ]]; then
        ...
        echo
        print_info "或者重新登录后使用完整路径: $install_dir/git-merge-push"
    fi
}

# 主安装函数
main() {
    print_info "开始安装 git-merge-push 全局命令..."
    ...
    # 创建符号链接
    if ! create_symlink "$install_dir" "$script_path" "git-merge-push"; then
        print_error "安装失败"
        exit 1
    fi
    ...
    echo "  git-merge-push              # 仅执行 git 操作"
    echo "  git-merge-push --release    # 执行 git 操作并触发发版"
    echo "  git-merge-push --help       # 显示帮助信息"
}

# 卸载函数
uninstall() {
    print_info "开始卸载 git-merge-push 全局命令..."
    
    # 获取安装目录
    local install_dir
    install_dir=$(get_install_dir)
    
    local link_path="$install_dir/git-merge-push"
...
show_help() {
    ...
    echo ""
    echo "选项:"
    echo "  install     安装 git-merge-push 全局命令（默认）"
    echo "  uninstall   卸载 git-merge-push 全局命令"
    echo "  -h, --help  显示此帮助信息"
    echo ""
    echo "功能:"
    echo "  将 git-merge-push.sh 安装为全局命令"
    echo "  支持在任何目录下执行 git-merge-push"
